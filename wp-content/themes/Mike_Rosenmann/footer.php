
<footer class="footer">
        <div class="row">
            <ul class="list-inline text-center footer-icons">
                <li><a href="#"><img class="" src="<?php echo get_template_directory_uri(); ?>/img/icons/instagram.svg" alt=""></a></li>
                <li><a href="#"><img class="" src="<?php echo get_template_directory_uri(); ?>/img/icons/messeger.svg" alt=""></a></li>
                <li><a href="#"><img class="" src="<?php echo get_template_directory_uri(); ?>/img/icons/facebook.svg" alt=""></a></li>
            </ul>
            <div class="col-md-8 col-md-offset-2 text-center footer-text">
                <p>Mike Rosenmann is a life coach. Aliquet euismod metus at vehicula. Phasellus facilisis diam ac tortor imperdiet congue. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce non sem lacinia.</p>
            </div>
        </div>
        <div class="row">
            <div class="text-center">
                <ul class="navi-footer">
                    <li><a href="#">Get In Touch</a></li>
                    <li><img class="point" src="<?php echo get_template_directory_uri(); ?>/img/icons/point.svg"></li>
                    <li><a href="#">About Mike</a></li>
                    <li><img class="point" src="<?php echo get_template_directory_uri(); ?>/img/icons/point.svg"></li>
                    <li><a href="#">Beast Mode</a></li>
                </ul>
            </div>
        </div>
    </footer>

